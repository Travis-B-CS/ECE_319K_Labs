...
int main1(void){ // use main1 to test your FIFO1
  ...
  myFifo_Init(13); // change 16 to match your FIFO size
...
void TIMG12_IRQHandler(void){
  ...
  if((TIMG12->CPU_INT.IIDX) == 1){ // this will acknowledge
    ...
    pos = ADCin(); // sample
    msg = Convert(pos); // convert to fixed point distance
    int CharOutputs[4] = {0, 0, 0, 0};
    for(int i = 0; i < 4; i++){
      CharOutputs[i] = (msg % 10);
      msg = msg / 10;
    }
    CharOutputs[0] = '<';
    UART1_OutChar(CharOutputs[0]);
    UART1_OutChar(CharOutputs[3]);
    UART1_OutChar(CharOutputs[2]);
    UART1_OutChar(CharOutputs[1]);// output 4-frame message
    GPIOB->DOUTTGL31_0 = GREEN; // toggle PB27 (minimally intrusive debugging)
  }
}
...
int main(void){ // main5
  ...
  ST7735_SetCursor(0,0); // move cursor to top left
    ...
    ones = UART2_InChar();
    tenths = UART2_InChar();
    hundreths = UART2_InChar();  // receive next three frames
    ReceiveCount++; // increment ReceiveCount
    }
    GPIOB->DOUTTGL31_0 = RED; // toggle PB26 (minimally intrusive debugging)
    printf("d=%d.%d%d cm", ones, tenths, hundreths); // output distance to ST7735
    PositionReceived = ones*1000+tenths*100+hundreths*10; // calculate PositionReceived from message
