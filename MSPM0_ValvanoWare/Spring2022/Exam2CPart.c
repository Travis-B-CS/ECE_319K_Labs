...
uint8_t Count_Points(Rect_t Shape, Point_t Points[], uint8_t n){
  int cnt = 0;
  for(int i = 0; i < n; i++){
    if((Points[i].x < Shape.topLeft.x || Points[i].x > Shape.botRight.x) && (Points[i].y < Shape.topLeft.y || Points[i].y > Shape.botRight.y)){
      cnt++;
    }
    else if(Points[i].x < Shape.topLeft.x || Points [i].x > Shape.botRight.x || Points[i].y < Shape.topLeft.y || Points[i].y > Shape.botRight.y){
      cnt++;
    }
  }
