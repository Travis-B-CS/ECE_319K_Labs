...
// Runs on MSPM0G3507
// Lab 7 
// Travis Beach and Jonathan Esqueda
...
// output to ST7735 0.000cm to 2.000cm
  ...
  printf("d=%d.%d%d%d cm",CharOutputs[3], CharOutputs[2], CharOutputs[1], CharOutputs[0]);
}
...
int main(void){ // main5
  ...
  __enable_irq();
  while(1){
    while(flag == 0){}// wait for semaphore
    ...
    ST7735_SetCursor(0,0); // move cursor to top
    OutFix(Position);// display distance in top row OutFix
    Time++;
    if((Time%15)==0){
